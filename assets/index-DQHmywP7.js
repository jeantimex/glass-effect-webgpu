...
fn fs_main(input: VertexOutput) -> @location(0) vec4f {
  ...
  if (distance_from_edge >= bezel_pixels) {
    // In article mode, fully transparent center
    if (uniforms.article_mode > 0.5) {
      return vec4f(0.0, 0.0, 0.0, 0.0);
    }
  ...
  // In article mode, only render specular rim effect
  if (uniforms.article_mode > 0.5) {
    let rim_alpha = (1.0 - bezel_t) * 0.3;  // Fade from edge inward

    // Calculate specular for rim highlight
    let spec_intensity = calculate_specular(
      distance_from_edge,
      bezel_pixels,
      direction,
      uniforms.specular_angle
    );
    let rim_color = vec3f(1.0);
    let final_alpha = rim_alpha + spec_intensity * uniforms.specular_opacity;

    return vec4f(rim_color * final_alpha, final_alpha);  // Premultiplied alpha
  }
